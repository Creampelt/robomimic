...
class RNNActorNetwork(RNN_MIMO_MLP):
    ...
    def output_shape(self, input_shape):
        ...
        TensorUtils.assert_size_at_dim(
            input_shape, size=T, dim=0, msg="RNNActorNetwork: input_shape inconsistent in temporal dimension"
        )
...
class RNNGMMActorNetwork(RNNActorNetwork):
    ...
    def forward(self, obs_dict, goal_dict=None, rnn_init_state=None, return_state=False):
        ...
        out = self.forward_train(
            obs_dict=obs_dict, goal_dict=goal_dict, rnn_init_state=rnn_init_state, return_state=return_state
        )
        if return_state:
            ad, state = out
...
class TransformerActorNetwork(MIMO_Transformer):
    ...
    def output_shape(self, input_shape):
        ...
        T = input_shape[mod][0]
        TensorUtils.assert_size_at_dim(
            input_shape, size=T, dim=0, msg="TransformerActorNetwork: input_shape inconsistent in temporal dimension"
        )
        return [T, self.ac_dim]
